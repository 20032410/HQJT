...
public class StringClass
{
    ...
    public static string makeFileName()
    {
       ...
       {
        string newFileName;
        string dateName = System.DateTime.Now.ToString("hhmmss");
        System.Random srd = new Random();
        int srdName = srd.Next(1000);
        newFileName = dateName + srdName.ToString();
        return newFileName;
    ...
    #endregion
    public static string htmlSub(string strContent)
    {
        //strContent = strContent.Replace(";", "");
        strContent = strContent.ToLower().Replace("■", ",");
        strContent = strContent.ToLower().Replace("∴", "'");
        ...
        {
            strContent = strContent.Substring(0, 30);
        }
        return strContent;
    }

    public static string htmlcuterSub(string strContent)
    {
        //strContent = strContent.Replace(";", "");
        strContent = strContent.ToLower().Replace("■", ",");
        strContent = strContent.ToLower().Replace("∴", "'");
    ...
    /// 替换URL特殊字符
    /// </summary> 
    public static string repUrl(string strTemp)
    {
        if (strTemp == null)
            strTemp = "";
        strTemp = strTemp.Replace("'", "");
    ...
    public static string EncryptMD5(string cleanString)
    {
        ...
        Byte[] hasheDBClassytes = ((HashAlgorithm)CryptoConfig.CreateFromName("MD5")).ComputeHash(clearBytes);
        return BitConverter.ToString(hasheDBClassytes);
    }
    #endregion
...
///然后接受的时候解密.这样能有效的防止sql注入攻击!!!Ò®æÙ
    ...
    #region  价格格格式
    public static string HtmlPrice(string all)
    {
        if (all.IndexOf(".") != -1)
        {
            if (all.Length - (all.LastIndexOf(".") + 3) > 0)
            {
                all = all.Substring(0, (all.LastIndexOf(".") + 3));
            }
        }
        else
        {
    ...
    public static string HtmlPrice1(string all1)
    {
        if (all1.IndexOf(".") != -1)
        {
            if (all1.Length - (all1.LastIndexOf(".")) > 0)
            {
                all1 = all1.Substring(0, (all1.LastIndexOf(".")));
            }
        }
        else
        {
    ...
    public static string GetProvence(string id)
    {
        string[] provences ={ "北京", "安徽", "山东", "江苏", "上海", "广东", "浙江", "福建", "重庆", "甘肃", "广西", "贵州", "海南", "河北", "河南", "黑龙江", "湖北", "湖南", "江西", "吉林", "辽宁", "内蒙古", "宁夏", "青海", "山西", "陕西", "四川", "天津", "新疆", "西藏", "云南", "香港", "澳门", "台湾", "国外与其它" };
        int ids = 0;
        if (int.TryParse(id, out ids))
        {
            return provences[ids];
        }
        else return "没有选择！";
    }

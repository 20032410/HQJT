...
strFrame+='<input id="txtObj" type="hidden" name="txtObj" />';
...
function setday(tt,obj) //主调函数
{
	...
	var th = tt;
	var ttop = tt.offsetTop;	//TT控件的定位点高    odatelayer.txtObj.value=tt.id//存放点击的文本框，以便外面调用ivan
	var thei = tt.clientHeight;	//TT控件本身的高
	var tleft = tt.offsetLeft;	//TT控件的定位点宽

	var ttyp = tt.type;	//TT控件的类型
    while (tt = tt.offsetParent) { // modified by justn.tan on 2007-01-24
		tn = tt.tagName.toUpperCase();		
        ttop += tt.offsetTop - (tn == "DIV"&&tt.scrollTop?tt.scrollTop:0); 
        tleft += tt.offsetLeft - (tn == "DIV"&&tt.scrollLeft?tt.scrollLeft:0); 
    }
    
	//while (tt = tt.offsetParent){ttop+=tt.offsetTop; tleft+=tt.offsetLeft;}
	WebHeight=document.body.offsetHeight;//文档页面的高度
	selfHeight=221;//自身的高度
	
	if(WebHeight-selfHeight<ttop)
	{
	    dads.top = (ttyp=="image") ? ttop+thei-selfHeight : ttop+thei+6-selfHeight;
	    dads.left = tleft;
	}
	else
	{	
